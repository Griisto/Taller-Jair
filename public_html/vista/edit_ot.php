...
if (!empty($_GET['id_ot'])) {
    $id_ot = $_GET['id_ot'];
    $sql = "SELECT * FROM ordenes_trabajo WHERE id = :id_ot";
    ...
    $sqlSpares = "SELECT rp.id,rp.nombre,ot.id FROM repuestos rp 
    JOIN repuestos_ot rt ON rp.id = rt.fk_repuesto 
    JOIN ordenes_trabajo ot ON rt.fk_ot = ot.id 
    WHERE ot.id=:idOt";
    ...
    FROM mano_obra mo 
    JOIN ot_mo ON mo.id = ot_mo.fk_mo
    JOIN marcas_vehiculo mv ON mo.vehiculo = mv.id
    WHERE ot_mo.fk_ot = :id_ot";
...
<form action="../controlador/edit_ot.php" method="post" id="form" class="form" enctype="multipart/form-data">
    <div class="info-ot">
        <div class="div-input">
