...
if (!empty($_GET)) {
    ...
    $querylabor = "SELECT trabajo,precio_trabajo FROM mano_obra mo 
    JOIN ordenes_trabajo ot 
    JOIN ot_mo otmo
    ON otmo.fk_ot = ot.id
    ON otmo.fk_mo = mo.id
    WHERE ot.id = :id_ot";
    $datelabor = $con->prepare($querylabor);
    $datelabor->bindParam(':id_ot', $id_ot);
    $datelabor->execute();
    $datelabor->fetchAll(PDO::FETCH_ASSOC);
    ...
    <div class="container-father">
        <div class="info">
            ...
            <div class="container-info info-mecanica">
                ...
                <?php foreach ($datelabor as $labor) : ?>
                    <p><?= $labor['trabajo'] ?>: <?= $labor['precio_trabajo'] ?></p>
                <?php endforeach; ?>
            </div>
        </div>
