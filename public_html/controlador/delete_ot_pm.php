...
if (!empty($_GET['id_ot']) && !empty($_GET['rute1'])) {
    ...
    $sql = "DELETE FROM ordenes_trabajo_pm WHERE id = :id_ot";
    $query = $con->prepare($sql);
    $query->bindParam(':id_ot', $id_ot);
    if ($query->execute()) {
        $message = "La orden de trabajo seleccionada se borro exitosamente";
        header("Location: ../vista/view_ot_pr.php?message=$message");
    }else{
        $message = "La orden de trabajo seleccionada no se borro exitosamente";
        header("Location: ../vista/view_ot_pr.php?message=$message");   
    }
}
$con = null;
