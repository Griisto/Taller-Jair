...
if (!empty($_POST)) {
    if (!empty($_POST['cliente']) && !empty($_POST['plaque']) && !empty($_POST['date_into']) && !empty($_POST['hour_into']) && !empty($_POST['date_exit']) && !empty($_POST['hour_exit']) && !empty($_POST['phone']) && !empty($_POST['id_usuario'])) {
        ...
        $sqlDeleteRelationHandLabor = "DELETE FROM ot_mo WHERE fk_ot = :id_ot";
        ...
        $sqlDeleteRelationSpares = "DELETE FROM repuestos_ot WHERE fk_ot =:id_ot";
        ...
        if (($queryDeleteRelationHandLabor->execute()) && ($queryDeleteRelationSpares->execute())) {
            $sql = "UPDATE ordenes_trabajo SET cliente=?, placa_vehiculo=?, fecha_ingreso=?,hora_ingreso=?, fecha_salida=?, hora_salida=?,telefono=?,cantidad_repuestos = ?, imagen =?, id_usuario =? WHERE id=?";
            ...
            if ($query->execute()) {
                ...
                if (!empty($handlabor)) {
                    ...
                    header("Location: https://" . $_SERVER['HTTP_HOST'] . "../vista/perfil_user.php?message=$message");
                }
                if (!empty($Spares)) {
